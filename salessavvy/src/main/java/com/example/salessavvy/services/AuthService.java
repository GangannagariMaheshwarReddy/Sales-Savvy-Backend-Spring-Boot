...
public class AuthService {
    ...
    public String generateToken(User user) {
        ...
        if (existingToken != null && !existingToken.getExpiresAt().isBefore(now)) {
    ...
    public JWTToken extractToken(String token) {
        // Look up token in DB and return entity if present and valid
        Optional<JWTToken> jwtToken = jwtTokenRepository.findByToken(token);
        if (jwtToken.isPresent() && jwtToken.get().getExpiresAt().isAfter(LocalDateTime.now())) {
            return jwtToken.get();
        }
        return null;
    }
